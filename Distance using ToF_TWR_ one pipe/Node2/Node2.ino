...
void loop() {
  ...
  if (radio.available()){
    ...
    Serial.print("Message Recieved From Node1 saying");Serial.println(message);
  }
  radio.openWritingPipe(adresse);     // Ouverture du tunnel en ECRITURE
  message="Hi!!";
  // Basculer en mode écriture
  radio.stopListening();  
  // Il faut extraire le retard resultant de ces deux instructions
  radio.write(&message,sizeof(message));   
  Serial.println("Acknologment sent to node 1"); // Pour acqusé la reception
  delay(5000);
  radio.stopListening();
}