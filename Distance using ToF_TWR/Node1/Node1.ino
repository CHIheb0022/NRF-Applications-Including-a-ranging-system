...
char message[16];                     // Avec cette librairie, on est "limité" à 32 caractères par message
...
void loop() {
  ...
  Serial.println("From Node1 to Node2 : " + message);
  // Basculer en mode lecture
  radio.startListening(); 
  if (!radio.available()) {                      
    Serial.println("Nothing is received"); 
    delayMicroseconds(1);                         
  }
  // Recevoir de la Noeud2 un message pour acquisé la reception // Timer Stop
  radio.read(&message,sizeof(message));
  sendTime=micros(); 
  Serial.println("From Node2 to Node1 : " + message);
